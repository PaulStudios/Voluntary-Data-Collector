...
import org.paulstudios.datasurvey.network.generateUniqueFileName
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            "STOP_SERVICE" -> stopSelf()
                ...
                getSharedPreferences("LocationService", MODE_PRIVATE)
                    .edit()
    ...
    private fun saveData() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val gpsDataList =
                    GPSDataList(userId, this@LocationService.gpsDataList, generateUniqueFileName())
